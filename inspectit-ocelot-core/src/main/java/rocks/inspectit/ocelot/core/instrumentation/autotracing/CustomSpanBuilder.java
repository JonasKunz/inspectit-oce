...
public class CustomSpanBuilder {
    ...
    public Span startSpan() {
        ...
        SpanContext context = SpanContext.create(parentContext.getTraceId(), id, parentContext.getTraceOptions(), parentContext.getTracestate());

        if (entryNanos != 0) {
            DummyClock clock = new DummyClock();
            clock.setValue(entryNanos);
            RecordEventsSpanImpl result = RecordEventsSpanImpl.startSpan(context, name, null, parentContext.getSpanId(), false, params, getStartEndHandler(), converter, clock);
            clock.setValue(exitNanos);
            return result;
        } else {
            Clock clock = Tracing.getClock();
            return RecordEventsSpanImpl.startSpan(context, name, null, parentContext.getSpanId(), false, params, getStartEndHandler(), converter, clock);
        }
    }
    ...
    private static TimestampConverter getTimestampConverter(RecordEventsSpanImpl span) {
        try {
